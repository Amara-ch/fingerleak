"""FingerLeak — Full multi-page web app (Tesla-grade UI)."""
...
st.set_page_config(
    page_title="FingerLeak — Seal the Leak",
    page_icon="◉",
    layout="wide",
...
with nc1:
    st.markdown(
        ...
        'FINGER<span class="dot">●</span>LEAK</div>',
        unsafe_allow_html=True
    )
with nc2:
...
# ═══════════════════════════════════════════════════════════════
def page_home():
    st.markdown('<div class="label"><span class="status-dot"></span>SYSTEM · ONLINE · MIT LICENSE</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.markdown(
            ...
            'Researchers proved it back in 2017 — and cameras have only gotten sharper since. '
        ...
        with b1:
            st.button("RUN SCANNER →", on_click=goto, args=("scanner",), use_container_width=True, key="hb1", type="primary")
        with b2:
    ...
    with col2:
        ...
        <div class="hero-panel">
            <div>
                <div class="label">⚠ LIVE THREAT INTEL</div>
    ...
    why = [
        ("THREAT", "Biometrics ≠ passwords",
         "You can change a password in seconds. You cannot change your fingerprint — ever. Once your ridges leak online, they can be replayed against any biometric system you'll ever use, for the rest of your life."),
        ("CONTEXT", "The peace-sign trap",
         "The most popular pose on social media — peace sign, hand wave, OK sign — exposes 4 fingertips at high resolution, directly facing the camera. Billions of these photos are uploaded daily."),
        ("SOLUTION", "Surgical filtering",
         "FingerLeak obfuscates only the fingertip pixels. Pose, identity, lighting, and scene remain untouched. The photo still looks like you — just without your biometric DNA exposed."),
    ]
    ...
    steps = [
        ("01", "DETECT",
         ...
         "MEDIAPIPE"),
        ("02", "SCORE",
         "A Gabor filter bank at 8 orientations recovers the latent ridge pattern. Combined with sharpness, crop size, and proximity, this yields a calibrated 0–100 FingerLeak-Score per finger.",
         "GABOR · OPENCV"),
        ("03", "PROTECT",
         "The chosen filter is applied selectively to fingertip bounding boxes only. We re-run the scorer on the protected output to verify ridge information has been destroyed — proof, not promise.",
         "SECURED"),
    ]
    ...
    users = [
        ...
        ("EVERYONE ELSE", "If you've ever posted a peace-sign selfie, OK sign, or thumbs-up — this matters to you."),
    ]
    ...
    with cc:
        st.button("LAUNCH SCANNER →", on_click=goto, args=("scanner",), use_container_width=True, key="cta_home", type="primary")
...
def page_scanner():
    ...
    if uploaded is None:
        ...
        with cs[0]:
            if sample.exists() and st.button("RUN SAMPLE", key="sample_btn", type="primary"):
                use_sample = True
        with cs[1]:
            st.caption("All processing local · no upload to server")
    ...
    with tm_col:
        ...
        st.caption(f"{len(before_scores)} fingertips · {n_crit + n_high} flagged high-risk")

    section_header("05", "EXTRACTION")
    st.markdown(
        '<p style="color:#888;margin-bottom:1.5rem;">Gabor filter bank reconstructs ridge structure '
        'from each fingertip — the same algorithm used by commercial fingerprint scanners.</p>',
        unsafe_allow_html=True
    )
    ...
    with cc1:
        st.markdown('<div class="label">BEFORE — EXPOSED</div>', unsafe_allow_html=True)
        st.image(cv2.cvtColor(img_disp, cv2.COLOR_BGR2RGB), use_container_width=True)
    with cc2:
        st.markdown(f'<div class="label" style="color:#00E676;">AFTER — {mode.upper()}</div>', unsafe_allow_html=True)
    ...
    with s3:
        st.markdown(f'<div class="card card-red"><div class="label">Δ REDUCTION</div><div class="tele-num red" style="margin-top:0.75rem;">−{drop:02d}</div></div>', unsafe_allow_html=True)
    ...
    for i, ((f, _, b), (_, _, a)) in enumerate(zip(ridges_before[:5], ridges_after[:5])):
        with pcols[i]:
            ...
            st.markdown('<div style="text-align:center;color:#FF3030;font-size:1.5rem;margin:0.5rem 0;">↓</div>', unsafe_allow_html=True)
            st.image(a_disp, clamp=True, use_container_width=True)
            red = b.ridge_response - a.ridge_response
            st.markdown(
                f'<div class="label" style="margin-top:0.5rem;">Δ RIDGE</div>'
                f'<div class="mono" style="font-size:1.1rem;color:#00E676;">−{red:.3f}</div>',
                unsafe_allow_html=True
            )
...
def page_research():
    ...
    st.markdown(
        ...
        'Every claim links to a primary source — papers, news, standards documents.</p>',
        unsafe_allow_html=True
    )
    st.markdown("<br>", unsafe_allow_html=True)

    tabs = st.tabs(["📜 PAPERS", "🔬 METHODOLOGY", "📰 NEWS", "📚 BIBLIOGRAPHY"])
    ...
    with tabs[0]:
        ...
        papers = [
            {
                ...
                "authors": "Hong, Wan, Jain · IEEE T-PAMI · 1998",
            ...
            },
            {
                ...
                "authors": "Isao Echizen et al. · NII Japan · 2017",
                "summary": ("First public demonstration that consumer phone cameras can capture "
                            "fingerprint data accurate enough to defeat biometric scanners — from photos "
            ...
            {
                "title": "NFIQ 2.0 — Fingerprint Image Quality",
                "authors": "NIST · 2016",
            ...
            },
            {
                ...
                "authors": "Maltoni, Maio, Jain, Prabhakar · Springer · 2022",
        ...
        for p in papers:
            ...
            with cc[1]:
                ...
                st.markdown(
                    f'<h3 style="margin-top:1rem;"><a href="{p["url"]}" target="_blank" style="color:#fff;">{p["title"]} →</a></h3>',
                    unsafe_allow_html=True
                )
                ...
                st.markdown(f'<a href="{p["url"]}" target="_blank" style="color:#FF3030;font-family:JetBrains Mono;font-size:0.8rem;letter-spacing:0.15em;">READ PAPER →</a>', unsafe_allow_html=True)
            st.markdown('<div class="div-thin"></div>', unsafe_allow_html=True)
    ...
    with tabs[1]:
        ...
        signals = [
            ("01", "0.35", "SHARPNESS",
             ...
             "REF = 200"),
            ("02", "0.20", "CROP SIZE",
             "Pixel side length of the cropped fingertip bounding box. Smaller crops mean the camera was farther away or the hand was tilted, both of which reduce ridge resolution below the matchable threshold. Reference 200 px corresponds to a fingertip ≈ 1cm in real space at 20cm distance.",
             "REF = 200 PX"),
            ("03", "0.30", "RIDGE STRENGTH",
             "Mean Gabor filter response across 8 orientations. Direct measurement of how strong the periodic ridge pattern is in the frequency domain. The single most predictive signal — if this is high, fingerprints are extractable regardless of other factors.",
             "REF = 0.30"),
            ("04", "0.15", "PROXIMITY",
        ...
        for i, (n, w, t, d, r) in enumerate(signals):
            ...
            with col:
                st.markdown(
                    ...
                    f'<div class="label">SIGNAL {n} · WEIGHT {w}</div>'
        ...
        thresholds = [
            ("LOW", "0–24", "#00E676", "Fingerprints not visible. Safe to share publicly."),
            ("MEDIUM", "25–49", "#FFB020", "Partial ridges visible. Filtering recommended."),
            ("HIGH", "50–74", "#FF6A30", "Strong ridge detail. Strongly advised to filter."),
            ("CRITICAL", "75–100", "#FF3030", "Matcher-grade quality. Do not share without filtering."),
        ]
        ...
        news = [
            ...
            },
            {
                ...
                "summary": ("Deep learning models can now reconstruct partial fingerprints with "
                            "80%+ matcher accept rates against real biometric systems — reshaping "
            ...
            },
            {
                ...
                "summary": ("Chaos Computer Club demonstrates fingerprint cloning from press photos "
                            "of a public figure. Proof that the attack is not theoretical — it's already "
        ...
        for n in news:
            ...
            with cc[1]:
                st.markdown(
                    ...
                    f'{n["date"]} · {n["source"]}</div>',
                    unsafe_allow_html=True
                )
                st.markdown(
                    f'<h3 style="margin-top:0.75rem;"><a href="{n["url"]}" target="_blank" style="color:#fff;">{n["title"]} →</a></h3>',
                    unsafe_allow_html=True
                )
                st.markdown(f'<p style="margin-top:1rem;">{n["summary"]}</p>', unsafe_allow_html=True)
                st.markdown(f'<a href="{n["url"]}" target="_blank" style="color:#FF3030;font-family:JetBrains Mono;font-size:0.8rem;letter-spacing:0.15em;">READ ARTICLE →</a>', unsafe_allow_html=True)
            st.markdown('<div class="div-thin"></div>', unsafe_allow_html=True)
    ...
    with tabs[3]:
        ...
        refs = [
            ...
            ("NIST.", "NFIQ 2.0 — Fingerprint Image Quality Standard.", "2016.", "https://www.nist.gov/services-resources/software/nfiq-2"),
            ...
            ("Google MediaPipe Team.", "MediaPipe Hands — On-Device Real-Time Hand Tracking.", "arXiv 2020.", "https://arxiv.org/abs/2006.10214"),
        ...
        for author, title, venue, url in refs:
            st.markdown(
                ...
                f'<div class="lc-val"><b>{author}</b> — {title}</div>'
                f'</a>',
                unsafe_allow_html=True
            )
...
def page_about():
    ...
    with a1:
        ...
        st.markdown(
            ...
            'vulnerable everyday photos are to biometric extraction — '
        ...
        st.markdown(
            ...
            'and privacy advocacy. Built to be auditable, reproducible, and useful — '
        ...
        st.markdown(
            '<p>Biometric authentication is everywhere — phones, banks, airports, government services. '
    ...
    with a2:
        ...
        <div class="card card-red">
            ...
            <p style="font-size:0.9rem;color:#bbb;">Independent researcher · Lahore, Pakistan 🇵🇰</p>
...
def page_contact():
    ...
    st.markdown(
        ...
        'or just to say hi — all welcome.</p>',
        unsafe_allow_html=True
    )
    st.markdown("<br>", unsafe_allow_html=True)
        ...
        with st.form("contact_form"):
            ...
            if submitted:
                if not name or not email or not message:
                    ...
                    st.success(f"✓ MESSAGE QUEUED · AMARA WILL REPLY TO {email}")
    ...
    faqs = [
        ...
        ("Is this open source?",
         "Yes — MIT licensed on GitHub. Audit, fork, contribute. Repository: github.com/Amara-ch/fingerleak"),
        ("Can I use FingerLeak in my own product?",
         "Absolutely. MIT license permits commercial use. We'd love to hear how — drop a message via the form above."),
...
st.markdown("""
<div class="footer">
    <div>FINGER<span style="color:#FF3030;">●</span>LEAK · 2026</div>
    <div>BUILT IN 6 DAYS · OPEN SOURCE · MIT</div>
    <div><a href="https://github.com/Amara-ch/fingerleak" target="_blank" style="color:#FF3030;">GITHUB →</a></div>
</div>
""", unsafe_allow_html=True)